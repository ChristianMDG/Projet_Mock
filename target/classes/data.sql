...
-- =============================================================================
CREATE UNIQUE INDEX IF NOT EXISTS uk_tarif_mobile_money_document_id
    ON public.tarif_mobile_money (document_id);

CREATE UNIQUE INDEX IF NOT EXISTS uk_frais_transaction_document_id
    ON public.frais_transaction (document_id);

CREATE UNIQUE INDEX IF NOT EXISTS uk_commission_document_id
    ON public.commission (document_id);

CREATE UNIQUE INDEX IF NOT EXISTS uk_tombana_fanaterana_document_id
    ON public.tombana_fanaterana (document_id);

-- =============================================================================
-- One-time data seed: Mobile Money tariffs (MVola, Orange, Airtel)
-- =============================================================================
INSERT INTO public.tarif_mobile_money
    (min_amount, max_amount, frais_retrait, frais_transfert,
     operator_name, document_id, locale, created_at, published_at)
VALUES
    -- MVOLA
    (100.00,      1000.00,    100.00,  70.00,  'MVOLA',  '65trw4f99xajhnhfnmq4ldcl', 'mg', NOW(), NOW()),
    ...
    -- ORANGE
    (200.00,      1000.00,    100.00,  65.00,  'ORANGE', '0is5b2bza3yg52whh44oco6k', 'mg', NOW(), NOW()),
    ...
    -- AIRTEL
    (100.00,      1000.00,    100.00,  50.00,  'AIRTEL', '4xz20cw38ulm7ua7g5fuwfuy', 'mg', NOW(), NOW()),
...
-- =============================================================================
INSERT INTO public.frais_transaction
    (operator_name, pourcentage, document_id, locale, created_at, published_at)
VALUES
    ('MVOLA',  1.50, 'mvola0000000000000000001', 'mg', NOW(), NOW()),
...
INSERT INTO public.commission
    (min_amount, max_amount, frais, koperative_id, document_id, locale, created_at, published_at)
...
FROM koperative k
CROSS JOIN (VALUES
    (0.00::numeric,     99999.00::numeric,  5000.00::numeric),
    (100000.00::numeric, 500000.00::numeric, 10000.00::numeric)
) AS tier(min_amount, max_amount, frais)
ON CONFLICT (document_id) DO NOTHING;

-- =============================================================================
-- One-time data seed: Tombana Fanaterana (estimated delivery rates)
-- =============================================================================
INSERT INTO public.tombana_fanaterana
    (min_weight, max_weight, frais, delivery_method, ville_id, document_id, locale, created_at, published_at)
...
FROM ville v
CROSS JOIN (VALUES
    (0.00::numeric,   5.00::numeric,  5000.00::numeric),
    (5.01::numeric,  20.00::numeric, 15000.00::numeric),
    (20.01::numeric,100.00::numeric, 30000.00::numeric)
) AS weight_tier(min_weight, max_weight, frais)
ON CONFLICT (document_id) DO NOTHING;