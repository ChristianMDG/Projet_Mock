...
-- =============================================================================
-- Migration: Add / Update montant_transfert in payment_transaction
-- montant_transfert = GREATEST(0, amount - COALESCE(commission_seats, 0))
-- =============================================================================
ALTER TABLE IF EXISTS payment_transaction
    ADD COLUMN IF NOT EXISTS montant_transfert NUMERIC(10, 2) DEFAULT 0;

DO
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM data_migration_log WHERE script_name = 'payment_transaction_montant_transfert_update') THEN

            UPDATE payment_transaction
            SET montant_transfert = GREATEST(0, amount - COALESCE(commission_seats, 0))
            WHERE montant_transfert = 0 OR montant_transfert IS NULL;

            INSERT INTO data_migration_log (script_name, executed_at) VALUES ('payment_transaction_montant_transfert_update', NOW());
        END IF;
    END
$$;