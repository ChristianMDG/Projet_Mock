...
public class ChatRoomService implements IChatRoomService {
    ...
    public ChatRoom createRoom(CreateChatRoomRequest request) {
        ...
        // Convert participant IDs to JSON
        if (request.getParticipantIds() != null) {
            try {
                String participantsJson = objectMapper.writeValueAsString(Arrays.asList(request.getParticipantIds()));
    ...
    public List<ChatRoom> getActiveRoomsForUser(String userId) {
        return chatRoomRepository.findActiveRoomsForUser(userId).stream()
    ...
    public List<ChatRoom> getAllActiveRooms() {
        return chatRoomRepository.findAllActiveRoomsOrderByLastActivity().stream()
                .map(ChatRoom::fromEntity)
                .collect(Collectors.toList());
    }
    ...
    public List<ChatRoom> findByType(ChatRoomType type) {
        return chatRoomRepository.findByTypeAndIsActiveTrue(type).stream()
                .map(ChatRoom::fromEntity)
                .collect(Collectors.toList());
    }
    ...
    private void addUserToRoom(ChatRoomEntity room, String userId) {
        try {
            List<String> participants = Arrays.asList(objectMapper.readValue(room.getParticipants(), String[].class));
    ...
    private void removeUserFromRoom(ChatRoomEntity room, String userId) {
        try {
            List<String> participants = Arrays.asList(objectMapper.readValue(room.getParticipants(), String[].class));
            List<String> updatedParticipants = participants.stream()
