...
import { useKoperativeBanner } from '@/hooks/cms.hooks';
import { HideOnMobile } from '@/shared';

const KoperativeBanner: React.FC = () => {
  const { data: content, isLoading } = useKoperativeBanner();
  ...
  if (!content?.data || Object.keys(content.data).length === 0) {
    return <MissingContent componentName="Koperative Banner" />;
  }
  ...
  return (
    <HeroSectionContainer component="section" aria-label={Title} sx={HERO_DIMENSIONS}>
      ...
      <HeroContentPanel component="article">
        ...
        <Typography variant="h5" component="h3" sx={{ mb: 1, color: 'secondary.main', fontWeight: 600 }}>
          {SubTitle}
        </Typography>
...
export default KoperativeBanner;
