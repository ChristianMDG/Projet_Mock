...
export default function RentalUtilitairesPage() {
  const { t } = useTranslation();
  const { data: utilitaires, isLoading } = useVehicles('UTILITAIRE');

  return (
    <>
      <Box component="main" sx={{ flexGrow: 1 }}>
        {/* Hero Section Utilitaires */}
        <Box
          sx={{
            ...
            backgroundImage: theme =>
              `linear-gradient(${alpha(theme.palette.primary.main, 0.8)}, ${alpha(theme.palette.primary.main, 0.9)}), url(https://images.unsplash.com/photo-1601584115197-04ecc0da31d7?auto=format&fit=crop&q=80)`,
        ...
        >
          <Container maxWidth="md">
            <Typography variant="h2" component="h1" sx={{ fontWeight: 800, mb: 2 }}>
              {t(Labels.rental_utilitaires_title)}
            </Typography>
            <Typography variant="h6" sx={{ fontWeight: 400, opacity: 0.9, mb: 4 }}>
              {t(Labels.rental_utilitaires_desc)}
            </Typography>
          </Container>
        </Box>
        ...
        <Container maxWidth="lg" sx={{ mb: 8 }}>
          <Box sx={{ mb: 4 }}>
            <Typography variant="h4" sx={{ fontWeight: 700, color: 'primary.main', mb: 1 }}>
              {t(Labels.rental_utilitaires_list_title)}
            </Typography>
            <Typography variant="body1" color="text.secondary">
              {t(Labels.rental_utilitaires_list_desc)}
            </Typography>
            <Divider sx={{ my: 3 }} />
          </Box>
          ...
          <Grid container spacing={3}>
            {isLoading
              ...
              : utilitaires?.map(vehicle => (
                  <Grid size={{ xs: 12, sm: 6, md: 4 }} key={vehicle.id}>
                    {/* For utilitaires, the link should probably go to a specific utilitaire details page, but we'll wire it to /vehicule/:id for now or the new specific one later */}
